...
object Writer {
  ...
  def createBuildTargetFile(
    ...
    for {
      ...
      buildFileContent <- createBuildTargetFileContents(
        buildHeader,
        ts,
        thirdPartyDirectory
      )
  ...
  def createBuildTargetFileContents(
      buildHeader: String,
      ts: List[Target],
      thirdPartyDirectory: DirectoryName
  ): Result[String] = {
    ...
    Traverse[List]
      .traverse(
        ...
        def kvOpt(key: String, valueOpt: Option[String], prefix: String = "")
            : String = valueOpt match {
          case Some(value) => kv(key, value, prefix)
          case None        => ""
        }

        def kListV(key: String, values: List[String], prefix: String = "")
            : String = {
      ...
      .map { case coord @ MavenCoordinate(g, a, v) =>
        ...
        val (sha1Str, sha256Str, serverStr, remoteUrl) = shas.get(coord) match {
          case Some(sha) =>
            ...
            val sha256Str =
              kvOpt("sha256", sha.binaryJar.sha256.map(_.toHex), ", ")
            // val url = sha.url
            val serverUrlStr =
              kvOpt("repository", servers.get(sha.binaryJar.serverId), ", ")
        ...
        val sourceStr = shas.get(coord).flatMap(_.sourceJar) match {
          case Some(sourceJar) =>
            ...
            // val url = sha.url
            val serverUrlStr =
              kvOpt("repository", servers.get(sourceJar.serverId), ", ")
        ...
        val comment = duplicates.get(coord.unversioned) match {
          case Some(vs) =>
            val status =
              ...
              else if (vs.map(_.destination.version).max == v)
                s"promoted to ${v.asString}"
        ...
        List(
          s"""$comment    {${kv("artifact", coord.asString)}""",
          s"""${kv(
            "lang",
            l.asString
          )}$sha1Str$sha256Str$serverStr$remoteUrl$sourceStr""",
          s"""${kv("name", coord.unversioned.toBazelRepoName(prefix))}""",
          s"""${kv("actual", actual.fromRoot)}""",
          s"""${kv("bind", coord.unversioned.toBindingName(prefix))}},"""
        ).mkString(", ")
      }
      .mkString("\n")
  ...
  def targets(
      g: Graph[MavenCoordinate, Unit],
      model: Model
  ): Either[NonEmptyList[TargetsError], List[Target]] = {
    ...
    check.right.flatMap { _ =>
      ...
      val thirdPartyVis =
        Target.Visibility.SubPackages(Label(None, Path(rootName.parts), ""))

      val allRootsUv = model.dependencies.roots.map(
        _.unversioned
      ) | model.dependencies.unversionedRoots
      def visibility(uv: UnversionedCoordinate): Target.Visibility =
        if (allRootsUv(uv)) Target.Visibility.Public
        else if (
          !model.options
            .flatMap { _.strictVisibility.map(_.enabled) }
            .getOrElse(true)
      ...
      val allUnversioned: Set[UnversionedCoordinate] =
        uvToVerExplicit.keySet.union(uvToRep.keySet)
       ...
       */
      val cache =
        scala.collection.mutable.Map[UnversionedCoordinate, Either[List[
          UnversionedCoordinate
        ], Either[NonEmptyList[TargetsError], Target]]]()
      def coordToTarget(
        ...
        def compute: Either[NonEmptyList[TargetsError], Target] = {
          ...
          def labelFor(
              u: UnversionedCoordinate
          ): Either[NonEmptyList[TargetsError], Label] =
            targetFor(u).right.map(_.name)

          Traverse[List]
            .traverse[E, Edge[MavenCoordinate, Unit], Label](deps) { e =>
              labelFor(e.destination.unversioned)
            }
            .right
            .flatMap { depLabelList =>
              val depLabels = depLabelList.toSet
              val (lab, lang) =
                Label
                  .replaced(u, model.getReplacements)
                  .getOrElse {
                    (
                      Label.parse(u.bindTarget(model.getOptions.getNamePrefix)),
                      langFn(u)
                    )
                  }
              // Build explicit exports, no need to add these to runtime deps
              Traverse[List]
                .traverse[E, UnversionedCoordinate, Label](
                  model.dependencies
                    .exportedUnversioned(u, model.getReplacements)
                    .right
                    .get
                )(labelFor(_))
                .right
                .map { uvexports =>
                  val (exports, runtime_deps) =
                    model.getOptions.getTransitivity match {
                      case Transitivity.Exports => (depLabels, Set.empty[Label])
                      case Transitivity.RuntimeDeps =>
                        (Set.empty[Label], depLabels)
                    }
      ...
      def getGenerateNeverlink(u: UnversionedCoordinate): Boolean =
        (for {
          m <- model.dependencies.toMap.get(u.group)
          projectRecord <- m.get(ArtifactOrProject(u.artifact.asString))
        } yield projectRecord.generateNeverlink.getOrElse(false))
          .getOrElse(false)

      Traverse[List].traverse[E, UnversionedCoordinate, Target](
        allUnversioned.toList
      )(targetFor(_))
    }
  }
}
