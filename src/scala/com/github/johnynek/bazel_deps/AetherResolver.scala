...
class AetherResolver(servers: List[MavenServer], resolverCachePath: Path)
    extends SequentialResolver[Try] {
  ...
  def resolverMonad: MonadError[Try, Throwable] =
    instances.try_.catsStdInstancesForTry
  ...
  private def request(m: MavenCoordinate, ml: Model): CollectResult = {
    ...
    collectRequest.setRoot(
      new Dependency(new DefaultArtifact(m.asString), "", false, exclusions)
    )
  ...
  def getShas(
            ...
            .map { case (coord, r) =>
              coord -> {
                val remoteRepository =
                  r.getRepository.asInstanceOf[RemoteRepository]
                val repositoryLayout = new Maven2RepositoryLayoutFactory()
                  .newInstance(session, remoteRepository)
                ...
                for {
                  ...
                  JarDescriptor(
                    url = Some(
                      new URI(remoteRepository.getUrl)
                        .resolve(
                          repositoryLayout
                            .getLocation(r.getArtifact, false)
                            .toString
                        )
                        .toString
                    ),
    ...
    Foldable[List].foldM(
      ...
      SortedMap.empty[MavenCoordinate, ResolvedShasValue]
    ) { case (m, (k, trySha)) =>
      trySha.map { sha =>
        m + (k ->
      ...
      case null =>
        Failure(
          ResolveFailure(
            ...
            m,
            ext,
            a.getExceptions.asScala.toList
          )
        )
      case art =>
        val f = art.getFile
  ...
  def addToGraph(
      deps: Graph[Node, Unit],
      dep: MavenCoordinate,
      m: Model
  ): Try[Graph[Node, Unit]] = Try {
  ...
  private class Visitor(initDeps: Graph[Node, Unit], model: Model)
    ...
    def coord(a: Dependency): MavenCoordinate = {
      ...
      MavenCoordinate(
        MavenGroup(artifact.getGroupId),
        MavenArtifactId(
    ...
    def addEdgeTo(d: Dependency): Boolean =
      (!d.isOptional) &&
        (d.getScope.toLowerCase match {
          ...
          case "runtime" =>
            true // TODO: we should only add these to runtime deps
          case "test" => false
          case "system" =>
            false // these should not be in maven, and should be handled by replacements
          case "import" =>
    ...
    /** Some maven artifacts are replaced, meaning we deal with them and their
      * dependencies manually. If this is true, never follow (but we do add the
      * edges to the node in such cases
      */
    ...
    def visitEnter(depNode: DependencyNode): Boolean = {
      logger.info(
        s"${depNode.getDependency} -> ${depNode.getChildren.asScala.toList.map(_.getDependency)}"
      )
      val dep = depNode.getDependency
      val shouldAdd = addEdgeTo(dep)

      /** unfollowed nodes are distinct from followed nodes. If project a has an
        * optional dependency on b, that does not mean another project does not
        * have a non-optional dependency
        */
      if (visited((dep, shouldAdd))) {
        ...
        if (shouldAdd) {
          ...
          logger.info(
            s"not adding dep: ($dep, ${dep.isOptional}, ${dep.getScope})"
          )
        }
