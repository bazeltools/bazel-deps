...
object Target {
  ...
  object Kind {
    def parse(str: String): Result[Kind] = str match {
      ...
      case other =>
        IO.failed(
          new IllegalArgumentException(s"unexpected library kind: $other")
        )
    }
  }
  ...
  object SourceList {
    def parseStringList(l: List[String]): Result[SourceList] = l match {
      ...
      case o =>
        IO.failed(new Exception(s"Unable to parse $o as a Source list."))
    }
    ...
    case class Explicit(srcs: Set[String]) extends SourceList {
      def render: Doc =
        if (srcs.isEmpty) Doc.empty
          ...
          renderList(Doc.text("["), srcs.toList.sorted, Doc.text("]"))(
            quote
          ).grouped
        }
    ...
    case class Globs(globs: List[String]) extends SourceList {
      ...
      def render: Doc =
        if (globs.isEmpty) Doc.empty
          ...
          val gstr =
            renderList(Doc.text("["), globs, Doc.text("]"))(quote).grouped
  ...
  object Visibility {
    ...
    case class SubPackages(of: Label)
        extends Visibility(s"${of.packageLabel.fromRoot}:__subpackages__")
    def parse(str: String): Result[Visibility] =
      str match {
        case "//visibility:public" => IO.const(Public)
        case e if e.endsWith(":__subpackages__") =>
          IO.const(
            SubPackages(Label.parse(e.dropRight(":__subpackages__".size)))
          )
        case o => IO.failed(new Exception(s"Unable to parse visibility: $o"))
      }
  }

  private[this] def parseLanguage(language: String): Result[Language] =
    language match {
      ...
      case e if e.startsWith("scala") =>
        e.split(":").toList match {
          case "scala" :: v :: Nil =>
            IO.const(Language.Scala(Version.apply(v), true))
          case "scala/unmangled" :: v :: Nil =>
            IO.const(Language.Scala(Version.apply(v), true))
          case o =>
            IO.failed(
              new Exception(s"Unable to parse scala configuration string: $e")
            )
        }
      case o => IO.failed(new Exception(s"Unable to parse language for $o"))
    }

  def fromListStringEncoding(
      rawSep: String,
      encodedContent: List[String]
  ): Result[Target] = {
    val seperator = Pattern.quote(rawSep)
    val resultV: Result[Map[String, List[String]]] = Traverse[List]
      .traverse(encodedContent) { ln: String =>
        val res: Result[(String, List[String])] =
          ln.split(seperator).toList match {
            ...
            case o =>
              IO.failed(new Exception(s"Unable to parse passed input: $o"))
          }
        res
      }
      .map(_.toMap)
    resultV.flatMap { entries =>
      def get(name: String): Result[List[String]] =
        entries.get(name) match {
          ...
          case None =>
            IO.failed(
              new Exception(
                s"Unable to find $name in input map, likely invalid data"
              )
            )
        }
      ...
      def getBoolean(name: String): Result[Boolean] =
        getS(name).flatMap {
          ...
          case o =>
            IO.failed(new Exception(s"unable to parse boolean as value $o"))
        }
      ...
      for {
        ...
        processorClasses <- get("processorClasses").map(
          _.map(ProcessorClass).toSet
        )
...
case class Target(
  ...
  /** This method is for encoding the target such that it can be represented in
    * bazel as a list of strings. Across the repository rule boundary we cannot
    * pass complex types and we don't want an external binary to be required to
    * do parsing/handling of a better format.
    *
    * Spec: <name>|||<type>|||<values> Where ||| is standing in for the
    * separator passed in Name is the field name as specified in the list below
    * Type is L for list of elements, B for a boolean or empty for a normal
    * string type Values are 0 or more strings that have no separator field in
    * them separated by the separator.
    *
    * @param separator
    *   this is the entry separator we've been passed in.
    * @return
    *   List of entries that describe this target encoded
    */
  def listStringEncoding(separator: String): Result[List[String]] = {
    def validate(strV: String): Result[Unit] =
      if (strV.contains("|")) {
        IO.failed(
          new Exception(
            s"Unable to encode ${strV} contains a | which isn't supported for bzl file encoding."
          )
        )
      } else IO.unit
  ...
  def toDoc: Doc = {
    ...
    /** e.g. scala_library( name = "foo", deps = [ ], exports = [ ],
      * runtime_deps = [ ], visibility = ["//visibility:public"] )
      */
    ...
    def sortKeys(tt: Doc, name: String, items: List[(String, Doc)]): Doc = {
      ...
      val sorted = items.collect {
        case (s, d) if !(d.isEmpty) => (s, d)
      }.sorted

      renderList(tt + Doc.text("("), nm :: sorted, Doc.text(")")) {
        case (k, v) =>
          k +: " = " +: v
      } + Doc.line
    }
    ...
    def labelList(ls: Set[Label]): Doc =
      renderList(
        Doc.text("["),
        ls.toList.map(_.asStringFrom(name.path)).sorted,
        Doc.text("]")
      )(quote)
    ...
    def renderPlugins(
        pcs: Set[ProcessorClass],
        exports: Set[Label],
        generatesApi: Boolean,
        licenses: Set[String]
    ): Doc =
      if (pcs.isEmpty) Doc.empty
      else
        processorClasses.toList
          .sortBy(_.asString)
          .map(renderPlugin(pcs, _, exports, generatesApi, licenses))
          .reduce((d1, d2) => d1 + d2)
    ...
    def generateTarget(neverlink: Boolean): Doc = {
      ...
      val (targetName, targetArgs) =
        if (neverlink)
          (
            name.name + "_neverlink",
            defaultArgs :+ (("neverlink", Doc.text("1")))
          )
        else (name.name, defaultArgs)

      sortKeys(targetType, targetName, targetArgs) + renderPlugins(
        processorClasses,
        exports,
        generatesApi,
        licenses
