...
class ModelTest extends FunSuite {
  test("specific versions sort correctly") {
    assert(
      VersionConflictPolicy.Highest
        .resolve(None, Set(Version("11.0.2"), Version("18.0"))) == Validated
        .valid(Version("18.0"))
    )

    assert(
      VersionConflictPolicy.Highest
        .resolve(Some(Version("18.1")), Set(Version("11.0.2"))) == Validated
        .valid(Version("18.1"))
    )
  }
  test("RC/M is less than final") {
    ...
    assert(
      ord.lt(Version("0.16.1-RC2"), Version("0.16.1")),
      "0.16.1-RC1 < 0.16.1"
    )
    assert(
      ord.lt(Version("0.16.1-RC2"), Version("0.16.10-RC4")),
      "0.16.1-RC1 < 0.16.10-RC4"
    )
    assert(
      ord.lt(Version("0.16.2-RC2"), Version("0.16.10-RC4")),
      "0.16.2-RC1 < 0.16.10-RC4"
    )
  }
    ...
    val deps = Dependencies(
      Map(
        MavenGroup("com.twitter") -> Map(
          ArtifactOrProject("finagle") -> ProjectRecord(
            lang,
            Some(Version("0.1")),
            Some(Set(Subproject(""), Subproject("core"))),
            None,
            None,
            None,
            None,
  ...
  test("coordinate naming") {
    val uc = UnversionedCoordinate(
      MavenGroup("com.twitter"),
      MavenArtifactId("finagle-core")
    )
    ...
    assert(
      uc.bindTarget(NamePrefix("")) == "//external:jar/com/twitter/finagle_core"
    )
    ...
    assert(
      uc.bindTarget(np) == "//external:jar/unique_com/twitter/finagle_core"
    )
  }

  test(
    "packaging and classifier are extracted properly parsed in MavenArtifactId"
  ) {
    def assertAll(
        id: MavenArtifactId,
        a: String,
  ...
  test("MavenArtifactId asString") {
    assert(
      ...
      ).asString == "foo:jar:some-classifier"
    )
    assert(
      MavenArtifactId(
        "foo",
        "dll",
        Some("some-classifier")
      ).asString == "foo:dll:some-classifier"
    )
    ...
    List("foo:jar:some-classifier", "foo", "foo:dll", "foo:dll:some-classifier")
      .foreach { s =>
        {
  ...
  test("MavenArtifactId addSuffix") {
    assert(
      MavenArtifactId("foo:dll:some-classifier")
        .addSuffix("_1")
        .asString == "foo_1:dll:some-classifier"
    )
    assert(MavenArtifactId("foo:dll").addSuffix("_1").asString == "foo_1:dll")
    assert(
      MavenArtifactId("foo:jar:some-classifier")
        .addSuffix("_1")
        .asString == "foo_1:jar:some-classifier"
    )
