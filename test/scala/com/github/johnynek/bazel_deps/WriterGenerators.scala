...
object WriterGenerators {
  ...
  val targetGen: Gen[Target] = for {
      language <- langGen
      name <- labelGen
      visibility <- labelGen.flatMap { id => Gen.oneOf(Target.Visibility.Public, Target.Visibility.SubPackages(id.copy(name = ""))) }
      kind <- Gen.oneOf(  Target.Library,
        Target.Import,
        Target.Test,
        Target.Binary
        )
      ...
      sources <- Gen.oneOf(Target.SourceList.Empty, Target.SourceList.Explicit(Set("abcd")), Target.SourceList.Globs(List("*.a.*java")))
    ...
    } yield Target(language,name,visibility, kind, deps, jars, sources, exports, runtimeDeps, processorClasses, generatesApi, licences, generateNeverLink)
}
